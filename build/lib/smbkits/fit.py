 ...
 Background Offset:\t    {b:5.2f}\n Final chi value:\t{c:5.2f} \n {message:20s}\n
	...
	def _HPchi2(self,X):	
        	f, self.fit_C, self.fit_F, self.fit_CF = self._HPmodel(X[0],X[1],X[2],X[3],X[4],self.en_crop)
       		xx = np.linspace(0,len(self.en_crop),len(self.en_crop))
       		bkg = xx * X[5]+X[6]
       		ft = (f + bkg)
	...
	def _HPExtend(self, X, en):
        	f, self.extend_C, self.extend_F, self.extend_CF = self._HPmodel(X[0],X[1],X[2],X[3],X[4],en)
       		xx = np.linspace(0,len(en),len(en))
       		bkg = xx * X[5]+X[6]
       		ft = (f + bkg)
